...
GeeseDB\footnote{\url{https://github.com/informagi/geesedb}, last accessed September 2025} is a prototype Python toolkit for information retrieval that leverages graphs as data structures, allowing metadata and graph-oriented data to be easily included in the ranking pipeline. The toolkit is designed to quickly set up first-stage retrieval and make it easy for researchers to explore new ranking models. 
...
hits = searcher.search_topic('fast dogs')
	\end{minted}
	\caption{Example on how to create a BM25 ranking for the query ``fast dogs'' that returns the top 10 documents. The Searcher class also takes the optional parameters $k_1$ and $b$, in this example it uses the default values $0.9$ and $0.4$ respectively~\citep{ATIRE}.}
