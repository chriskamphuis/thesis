...
A data graph is a type of graph where vertices have data. \Cref{data-graph} shows an example of a (directed) graph where every vertex is labeled with a number. These graphs are for example used in the famous PageRank algorithm~\citep{pagerank}. In this algorithm every vertex represents a webpage, and a directed edge exists between a node $a$ to $b$, if there is a link on website $a$ that points to website $b$. Then by randomly walking over this graph, or visiting a new website at random it is possible to count how often a node has been visited. The number of visits corresponds to the PageRank score. The PageRank scores in a graph correspond to the probabilities found in a Markov process in equilibrium.  

\paragraph{Weighed Graph}
A weighted graph is a type of graph where edges have data. \Cref{weighted-graph} shows an example of a (directed) graph where every edge is labeled with a number. These kinds of graphs are for example used to calculate the shortest distance between nodes. These kinds of graphs are, for example, useful for navigation systems. If we need to travel from location $x_1$ to location $x_3$ there are two possible paths. The path that goes from $x_1$ directly to $x_3$ is shorter compared to going from $x_1$ to $x_3$ via $x_2$. When graphs are larger, algorithms like Dijkstra's algorithm can find the shortest path over weighted graphs efficiently.

\paragraph{Pregel/Giraph Graph}
A obvious extension is, of course, adding data to both the vertices and edges. In the context we refer to them as Pregel/Giraph graphs, as these systems implement these kinds of graphs. There are some algorithms that need graphs to have both edge and vertex data. An example is the node2vec algorithm, which generates vector representations of the vertices of the graph. These graphs are used for various machine learning applications. 
...
Research Description Framework (RDF) graphs are directed graphs where edges have labels, and where multiple edges may exists between a pair of nodes. Graphs that allow multiple edges to exists between a pair of nodes are also called \emph{multi-graphs}.
They are a standard by the World Wide Web Consortium (W3C). The idea behind the RDF graphs is they the are a collection of so called triples, these triples contain a \emph{subject}, \emph{predicate}, and \emph{object}. The predicate describes the relation (edge) from the subject (node) to the object (node): a collection of triples forms a graph. RDF graphs are used a lot to represent knowledge graphs, which have many applications in information retrieval. It is, for example, possible to generate entity cards on top of search engine result pages using knowledge graphs. \Cref{rdf-graph} shows an example of a small RDF graph. 
